...
corepack enable
(export PATH="/root/.local/share/pnpm/bin:$PATH" ; pnpm config set registry -g "$BUILD_NPM_MIRROR")
...
echo "export PATH=\$PATH:$BUILD_AGENT_USER_HOME/.local/share/pnpm/bin" | sudo -u "$BUILD_USER_NAME" tee -a "$BUILD_USER_HOME/.bashrc.d/$(basename "$(pwd -P)").sh"
